...
WORK_STATE tls_post_process_client_hello(SSL_CONNECTION *s, WORK_STATE wst)
{
    ...
    // SSLfatal(s, TLS1_AD_DOWNGRADE_DETECTED, SSL_R_TLS13_DOWNGRADE_DETECTED);
    //     goto err;
...
CON_FUNC_RETURN tls_construct_server_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    /* Temporary forced downgrade detection alert for testing */
    // if (1 == 1) {
    //     SSLfatal(s, TLS1_AD_DOWNGRADE_DETECTED, SSL_R_TLS13_DOWNGRADE_DETECTED);
    //     return CON_FUNC_ERROR;
    // }
    ...
    /* ---------- server-side persistent downgrade check (TLS 1.3) ------ */
    // if (SSL_IS_TLS13(s)) {
            /* ------------------- Downgrade check ------------------- */

        SSL_CTX *ctx = SSL_CONNECTION_GET_CTX(s);
        DOWNGRADE_STORE *store = ssl_ctx_get_store(ctx, 0);

        if (store != NULL && store->last_version != 0) {

            if ( s->s3.tmp.new_cipher == NULL) {
                ssl3_send_alert(s, SSL3_AL_FATAL, TLS1_AD_INTERNAL_ERROR);
                return 0;
            }

            /* Version downgrade */
            if (s->version < store->last_version) {
                SSLerr(0, SSL_R_TLS13_DOWNGRADE_DETECTED);
                ssl3_send_alert(s, SSL3_AL_FATAL, TLS1_AD_DOWNGRADE_DETECTED);
                return 0;
            }

            /* Cipher downgrade */
            {
                int cur_bits = SSL_CIPHER_get_bits(s->s3.tmp.new_cipher, NULL);

                if (store->last_cipher_bits != 0) {
                    if (cur_bits < store->last_cipher_bits) {
                        SSLerr(0, SSL_R_TLS13_DOWNGRADE_DETECTED);
                        ssl3_send_alert(s, SSL3_AL_FATAL, TLS1_AD_DOWNGRADE_DETECTED);
                        return 0;
                    }
                } else if (store->last_cipher_id != 0) {
                    if (s->s3.tmp.new_cipher->id < store->last_cipher_id) {
                        SSLerr(0, SSL_R_TLS13_DOWNGRADE_DETECTED);
                        ssl3_send_alert(s, SSL3_AL_FATAL, TLS1_AD_DOWNGRADE_DETECTED);
                        return 0;
                    }

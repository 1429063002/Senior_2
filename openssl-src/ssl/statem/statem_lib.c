...
#include "../downgrade_store.h"
#include <openssl/x509.h>
#include <openssl/trace.h>
#include <openssl/encoder.h>
// #include "internal/cryptlib.h"
// #include "ssl/ssl_local.h"   /* for SSL_CONNECTION macros and internal types */
...
/* ------------------- Downgrade store helpers ------------------- */

static int downgrade_ex_idx = -1;

int get_downgrade_ex_index(void)
{
    if (downgrade_ex_idx != -1)
        return downgrade_ex_idx;

    downgrade_ex_idx = SSL_CTX_get_ex_new_index(0, NULL, NULL, NULL, NULL);
    return downgrade_ex_idx;
}

DOWNGRADE_STORE *ssl_ctx_get_store(SSL_CTX *ctx, int create)
{
    int idx = get_downgrade_ex_index();
    DOWNGRADE_STORE *st;

    if (idx < 0)
        return NULL;

    st = (DOWNGRADE_STORE *)SSL_CTX_get_ex_data(ctx, idx);
    if (st == NULL && create) {
        st = OPENSSL_zalloc(sizeof(*st));
        if (st == NULL)
            return NULL;

        SSL_CTX_set_ex_data(ctx, idx, st);
    }
    return st;
}

/* ------------------- End downgrade helpers --------------------- */

// /*
//  * Per-SSL_CTX in-memory store for the last observed negotiated parameters.
//  * We attach this via SSL_CTX ex_data to the SSL_CTX used by the server
//  * so it persists across connections in this server process.
//  */
// typedef struct {
//     int     last_version;
...
// /* helper: lazily allocate the ex_data index */
// static int get_downgrade_ex_index(void)
// {
//     if (downgrade_ex_idx != -1)
//         return downgrade_ex_idx;

//     /* create a context ex_data index (no callbacks required) */
//     downgrade_ex_idx = SSL_CTX_get_ex_new_index(0, NULL, NULL, NULL, NULL);
//     return downgrade_ex_idx;
// }

// /* helper: retrieve (or optionally create) the store for a given SSL_CTX */
// static DOWNGRADE_STORE *ssl_ctx_get_store(SSL_CTX *ctx, int create)
// {
//     int idx = get_downgrade_ex_index();
//     DOWNGRADE_STORE *st;
...
//         if (st == NULL)
//             return NULL;
//         /* defaults already zeroed: last_version == 0 means "no previous" */
//         SSL_CTX_set_ex_data(ctx, idx, st);
//     }
//     return st;
// }
                                ...
                                int clearbufs, int stop)
{
    ...
    if (clearbufs) {
        ...
        /* ---- store negotiated parameters on the server-side SSL_CTX ---- */
        /* Save negotiated parameters for next handshake (server side only) */
        if (s->server && s->s3.tmp.new_cipher != NULL) {
            SSL_CTX *ctx = SSL_CONNECTION_GET_CTX(s);
            DOWNGRADE_STORE *store = ssl_ctx_get_store(ctx, 1);

            if (store != NULL) {
                store->last_version = s->version;
                store->last_cipher_id = s->s3.tmp.new_cipher->id;
                store->last_cipher_bits =
                    SSL_CIPHER_get_bits(s->s3.tmp.new_cipher, NULL);
            }
        }
        /* ---- end store block ---- */
        s->init_num = 0;
